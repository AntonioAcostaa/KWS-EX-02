...
import { Select, Tooltip } from "@intility/bifrost-react";
...
async function loadPolar() {
  return await loadWtmsSource("/KWS-EX-02/arctic-sdi.xml", {
...
export function BaseLayerDropdown() {
  ...
  return (
    <Tooltip
      content={
        <div>
          <p>Map projection: {mapProjection}</p>
          <p>Layer projection: {selectedLayerProjection}</p>
        </div>
      }
      state="neutral"
    >
      <div>
        <Select
          ...
          value={baseLayerOptions.find((l) => l.id === selectedLayer.id)}
          style={{ width: "200px" }}
        />
      </div>
    </Tooltip>
